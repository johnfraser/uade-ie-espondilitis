...
public class DatosPaciente extends JPanel implements ActionListener, WindowListener {
	...
	public DatosPaciente(MainFrame frame) {
		...
		rdb_paciente_nuevo_si.setBounds(APP_WINDOW_X / 2 + 150, current_y, APP_WINDOW_X / 2 - padding_controles,
				alto_controles);
		...
		btBuscarCliente.setBounds(APP_WINDOW_X / 2 - 100, current_y, APP_WINDOW_X / 2 - padding_controles - 100,
				alto_controles);
		add(btBuscarCliente);
		...
		current_y = current_y + alto_controles;
		/*
		 * btGuardar = new JButton("Guardar"); btGuardar.addActionListener(this);
		 * btGuardar.setVisible(false); btGuardar.setBounds(APP_WINDOW_X / 2 - ( 150 /
		 * 2) ,current_y, 150 , alto_controles); add(btGuardar);
		 */
		current_y = current_y + alto_controles * 2;
	...
	private void cargarDiagnosticos_anteriores() {
		...
		lbl_diagnosticos_anteriores.setBounds(current_x, current_y,
				MainFrame.APP_WINDOW_X / 2 - MainFrame.padding_controles, MainFrame.alto_controles);
		...
		cmb_diagnosticos_anteriores.setBounds(MainFrame.APP_WINDOW_X / 2, current_y, MainFrame.padding_radio,
				MainFrame.alto_controles);
		...
		bt_diagnosticos_anteriores.setBounds(APP_WINDOW_X / 2 - 100, current_y,
				APP_WINDOW_X / 2 - padding_controles - 100, alto_controles);
	 ...
	 * MainFrame MainFrame;
	 * 
	 * //Create and set up the window. frame = new JFrame(APP_NAME + " "+
	 * APP_VERSION); frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	 * 
	 ...
	 * 
	 * Dimension screen = Toolkit.getDefaultToolkit().getScreenSize(); int x = (int)
	 * ((screen.getWidth() - frame.getWidth()) / 2); int y = (int)
	 * ((screen.getHeight() - frame.getHeight()) / 2);
	 * 
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		/*
		 * if (arg0.getSource() == btGuardar) { btGuardar(); }
		 */
	...
	private void btBuscarCliente() {
		...
		if (paciente != null) {
			...
			MainFrame.consulta = new Consulta();
			MainFrame.consulta.paciente = new Paciente(paciente.id_paciente, paciente.dni, paciente.nombre,
	...
	private void bt_diagnosticos_anteriores() {
		...
		if (cmb_diagnosticos_anteriores.isEnabled()) {
			if (cmb_diagnosticos_anteriores.getItemCount() > 0) {
				...
				// ResultadosAnteriores resultadosAnteriores = new
				// ResultadosAnteriores(frame,c);
	...
	private boolean GuardarPaciente() {
		...
		if (!encontroPacienteExistente) {
			...
			if (paciente_existente != null) {
				...
				id_paciente_actual = dbHelper.getUltimoId(DBHelper.TABLE_Paciente_NAME,
						DBHelper.TABLE_Paciente_id_paciente) + 1;
			}
		...
		if (!pacienteYaExisteError) {
			...
			if (encontroPacienteExistente) {
				if (MainFrame.consulta.paciente.nombre.compareTo(tld_nombre.getText()) != 0
					...
					JOptionPane.showMessageDialog(frame, strMensaje, "Datos del paciente",
							JOptionPane.INFORMATION_MESSAGE);
				}
			} else {
				MainFrame.consulta = null;
				MainFrame.consulta = new Consulta();
				MainFrame.consulta.paciente = new Paciente(id_paciente_actual, Integer.parseInt(tld_dni.getText()),
						tld_nombre.getText(), tld_apellido.getText(), Integer.parseInt(tld_edad.getText()), sexo);
	...
	public void windowClosing(WindowEvent e) {
		...
		int jOptionResult = JOptionPane.showOptionDialog(frame, strMsg, "Consulta", JOptionPane.YES_NO_OPTION,
